...
function collect_new() {
    ...
    try {
       sh.execSync(tvnamer + ' -c ' + __dirname + '/tvnamer.collect_new.json "' +
           config.new_media_in_dir + '"');
    }
    catch (e) {
    }
...
function move_to_tree() {
  ...
  // Ask tvnamer to move files to the library tree
    try {
       sh.execSync(tvnamer + ' -c ' + __dirname + '/tvnamer.move_to_tree.json "' +
         config.new_media_temp_dir + '"');
    }
    catch (e) {
    }
